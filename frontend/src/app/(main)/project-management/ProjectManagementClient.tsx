...
export default function ProjectManagementClient({
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      setLoading(true);
      try {
        await loadProjectDetail(projectIdProp);
        if (cancelled) return;
        try {
          const [c, te] = await Promise.all([pmApi.coders(), pmApi.testers()]);
          if (!cancelled) {
  ...
  }, [projectIdProp, loadProjectDetail]);
  ...
  function openProject(p: ITProject) {
    router.push(`/project-management/${p.id}`);
  }

  async function refreshActive() {
    if (activeProject || projectIdProp) {
      const id = activeProject?.id ?? projectIdProp!;
